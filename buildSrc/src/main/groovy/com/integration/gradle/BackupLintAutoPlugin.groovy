...
class BackupLintAutoPlugin implements Plugin<Project> {
...
//        throw new GradleException("目前只支持 android和android-library插件配置过的")
    ...
    private void applyTask(Project project, DomainObjectCollection<BuildType> variants) {
        ...
        // --- 统一 自动添加aar依赖开始（要把jar上传到远程maven服务） ---
        //addAar(project)

        //a 先拷贝自定义lint规则jar到本地目录
        //copyToGoalDir(project)
        println "2-------- 拷贝lint.jar 到${project.lintAutoInfo.goalJarDir}，目录不存在就新建 ----- （由于该目录属于隐藏目录所以不成功）"
        copyToHiddenDir(project)
        ...
        variants.all { variant ->
            ...
            if (variantName != "Debug"){
                ...
                println "3-------- 获取每个model对应的lint任务 ${project.tasks.getByName("lint" )}"
                ...
                lintTask.doFirst {
                    ...
                    if (lintFile.exists()) {
                        ...
                        println("5-2 ------ 目标文件不存在，直接把文件转移到model指定目录")
    ...
    private void addAar(Project project) {
        def lintDependencies = ":lintaar"
        //a1 配置Project的dependencies，默认都自动加上自定义lint检测的aar包
        project.dependencies {
            //如果是android application
            if (project.getPlugins().hasPlugin(AppPlugin)) {
                compile(lintDependencies) {
                    force = true
                }
            } else {
                provided(lintDependencies) {
                    force = true
                }
            }
        }
    ...
    private void copyToHiddenDir(Project project){
        File sourceFile = project.getParent().file("${project.lintAutoInfo.sourceJarPath}")
        Runtime run = Runtime.getRuntime()
        try {
            run.exec("chmod +777 ${project.lintAutoInfo.goalJarDir}")
            run.exec("mkdir ${project.lintAutoInfo.goalJarDir}")
        }catch (Exception e){
            e.printStackTrace()
        }
        try {
            run.exec("cp ${sourceFile.getAbsolutePath()} ${project.lintAutoInfo.goalJarDir}")
        }catch (Exception e){
            e.printStackTrace()
        }
    }
